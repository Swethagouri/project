...
def add_to_cart():
    ...
    cart_item = {
        ...
        'details': data.get('details', None)  # Store custom cake details if present
    }
...
def create_order():
    ...
    try:
        ...
        delivery_date = datetime.strptime(f"{data.get('delivery_date')} {data.get('delivery_time')}", '%Y-%m-%d %H:%M')
        
        # Get cart items and ensure custom cake details are preserved
        cart_items = session['cart']
        for item in cart_items:
            if 'details' in item and item['details']:
                # Ensure delivery preferences from the custom cake form are preserved
                item['details']['deliveryDate'] = data.get('delivery_date')
                item['details']['deliveryTime'] = data.get('delivery_time')
        ...
        order = Order(
            ...
            address=data.get('address') if data.get('delivery_type') == 'delivery' else 'Pickup',
            delivery_date=delivery_date,
            delivery_type=data.get('delivery_type'),
            items=cart_items,  # Use the updated cart_items with preserved details
